...
- Codex maps both `opus` and `sonnet` tiers to `gpt-5.4`; quality differences are expressed through reasoning effort
- OpenCode currently assumes `gpt-5.4` as the default runtime model; LOW/MEDIUM/HIGH act as reasoning-profile hints until explicit model overrides are surfaced
