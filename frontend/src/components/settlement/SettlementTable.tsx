        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100">
            {isLoading ? (
              ...
              const displayTitle = s.title || s.description || `정산 #${s.id}`;
              return (
                <tr key={s.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => onSelect(s)}>
                  <td className="px-6 py-4">
                    <div>
                      <p className="font-medium text-gray-800">{displayTitle}</p>
                      {s.contract_id && <p className="text-sm text-gray-500">계약 #{s.contract_id}</p>}
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    {s.model_name && (
                      <div className="flex items-center gap-1.5 text-sm">
                        <User className="w-3.5 h-3.5 text-gray-400" />{s.model_name}
                      </div>
                    )}
                  </td>
