...
export default function SettlementFormModal({ mode, initial, onClose, onSuccess }: Props) {
  ...
  const inputClass = 'w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500';
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-hidden">
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-800">{mode === 'edit' ? '정산 수정' : '정산 등록'}</h2>
              <p className="text-sm text-gray-500">{mode === 'edit' ? '정산 내역을 수정합니다' : '새로운 정산 내역을 등록합니다'}</p>
            </div>
          </div>
