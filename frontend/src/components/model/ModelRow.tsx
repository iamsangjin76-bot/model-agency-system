...
export default function ModelRow({ model, selected, onToggle, onDelete }: Props) {
  ...
  const profileImg = (size: string) => (
    <div className={`${size} rounded-xl bg-gray-200 overflow-hidden flex-shrink-0`}>
      {model.profile_image ? (
  ...
  return (
    <>
      {/* ── Card view: below lg ───────────────────────────────────── */}
      <div className="lg:hidden flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
        <input
          type="checkbox"
          checked={selected}
        ...
        <div className="flex-1 min-w-0">
          ...
          {model.name_english && (
            <p className="text-xs text-gray-400 truncate">{model.name_english}</p>
          )}
      ...
      <div className="hidden lg:grid lg:grid-cols-12 gap-4 px-6 py-4 hover:bg-gray-50 transition-colors">
