  ...
  return (
    <div className="lg:col-span-1">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 sticky top-6">
        ...
        {/* Main profile image */}
        <div
          className={`aspect-[3/4] min-h-[200px] bg-gray-100 dark:bg-gray-700 rounded-xl overflow-hidden mb-4 relative group ${previewImage ? 'cursor-pointer' : ''}`}
          onClick={() => previewImage && onPreviewChange(null)}
        >
          {displayedImage ? (
            <img src={displayedImage} alt="Profile" className="w-full h-full object-cover" />
          ) : (
            <div className="w-full h-full flex flex-col items-center justify-center text-gray-400 dark:text-gray-500 gap-1 px-4">
              <Camera className="w-12 h-12 mb-1 text-gray-300 dark:text-gray-600" />
              <p className="text-sm font-medium text-gray-500 dark:text-gray-400">프로필 사진</p>
              <p className="text-xs text-gray-400 dark:text-gray-500 text-center">800 × 1200px 권장</p>
              <p className="text-xs text-gray-400 dark:text-gray-500 text-center">JPEG, PNG, WebP / 최대 10MB</p>
            </div>
          )}
          {!previewImage && (
            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
              <label
                className="px-4 py-2 bg-white dark:bg-gray-800 rounded-lg font-medium text-gray-800 dark:text-gray-100 flex items-center gap-2 cursor-pointer"
        ...
        {/* File info */}
        {currentFileInfo && (
          <p className="text-xs text-gray-500 dark:text-gray-400 text-center mt-1 mb-2 truncate px-1" title={currentFileInfo.name}>
        ...
        <div className="grid grid-cols-4 gap-2">
          {[...Array(4)].map((_, i) => {
            ...
            return (
              <div
                key={i}
                className={`aspect-square bg-gray-100 dark:bg-gray-700 rounded-lg overflow-hidden relative group transition-all ${img ? `cursor-pointer ${isActive ? 'ring-2 ring-purple-500' : 'hover:ring-2 hover:ring-purple-400'}` : 'hover:ring-2 hover:ring-purple-400'}`}
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500 text-center mt-2">JPEG, PNG, WebP / 최대 10MB</p>
        ...
        <div className="mt-4">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            />
            <span className="font-medium text-gray-700 dark:text-gray-200">활성 상태</span>
          </label>
        </div>
      </div>
    </div>
