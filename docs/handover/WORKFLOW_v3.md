...
각 필드의 허용값은 **훅 규칙 파일(`.claude/rules/autopus/lore-commit.md`)에 명시**돼 있습니다. 현재 훅 자체는 값 열거를 검증하지 않지만, 장래 강화 시 자동 거부될 수 있으므로 처음부터 정답을 씁니다.

| 필드 | 허용값 | 의미 |
|---|---|---|
| `Constraint` | 자유 텍스트 (**필수**) | 이 변경이 지켜야 할 제약 또는 설계 경계 |
| `Confidence` | `high / medium / low` | 변경의 정확성에 대한 자신감 |
| `Scope-risk` | `local / module / system` | **영향 범위** (위험도 아님) |
| `Reversibility` | `trivial / moderate / difficult` | 되돌리기 난이도 |
| `Directive` | 자유 텍스트 (`none` 가능) | 후속 작업 방향 |
| `Tested` | 자유 텍스트 | 어떻게 검증했는지 |
| `Not-tested` | 자유 텍스트 (`-` 가능) | 검증 안 한 부분 |
| `Related` | 자유 텍스트 (SPEC ID 권장) | 관련 SPEC 또는 이슈 |

#### Scope-risk 판단 기준

- `local` — 단일 파일 또는 단일 함수 수준. 다른 코드에 영향 없음. 예: 오타 수정, 주석 추가, 단일 함수 리팩토링
- `module` — 한 모듈/레이어 수준. 같은 모듈 내 여러 파일 영향. 예: 한 API 엔드포인트 시그니처 변경, 단일 컴포넌트 대대적 리팩토링
- `system` — 여러 모듈 교차 또는 시스템 전반 영향. 예: DB 스키마 변경, 인증 시스템 수정, 크로스 레이어 변경
...
> **최종 수정**: 2026-04-16 (Phase C v3 — Lore 훅 실전 검증 반영: Scope-risk 허용값 정정, Trailer 필드 의미 설명 추가)
